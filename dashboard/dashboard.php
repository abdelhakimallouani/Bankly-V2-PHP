...
$sql = "SELECT COUNT(id_client) AS nb_clients 
FROM client";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$nb_clients = $row['nb_clients'];

$sql = "SELECT COUNT(*) AS nb_clients 
FROM client";
$res = mysqli_query($conn, $sql);
$nb_clients = mysqli_fetch_assoc($res)['nb_clients'];

$sql = "SELECT COUNT(*) AS nb_comptes 
FROM compte WHERE statut = 'actif'";
$res = mysqli_query($conn, $sql);
$nb_comptes = mysqli_fetch_assoc($res)['nb_comptes'];

$sql = "SELECT SUM(solde) AS solde_total 
FROM compte";
$res = mysqli_query($conn, $sql);
$solde_total = mysqli_fetch_assoc($res)['solde_total'];

$sql = "SELECT SUM(montant) AS total_depot
        FROM transaction
        WHERE type = 'depot'";
$res = mysqli_query($conn, $sql);
$total_depot = mysqli_fetch_assoc($res)['total_depot'];

$sql = "SELECT SUM(montant) AS total_retrait
        FROM transaction
        WHERE type = 'retrait'";
$res = mysqli_query($conn, $sql);
$total_retrait = mysqli_fetch_assoc($res)['total_retrait'];

$sql = "SELECT COUNT(*) AS nb_transactions FROM transaction";
$res = mysqli_query($conn, $sql);
$nb_transactions = mysqli_fetch_assoc($res)['nb_transactions'];
    ...
    <section class="fixed left-0 top-0 h-full w-70 bg-[#012A7D] border-r border-gray-200 shadow-sm p-6 flex flex-col">
        ...
        <div class="border-t border-gray-200 pt-2">
            <div class="flex items-center space-x-3 mb-2">
                <div class="text-[#E0E0E0] hover:bg-[#ffffff3d] rounded-lg w-full p-2">
    ...
    <section class="ml-70 p-8 bg-[#F9FAFB]">
        ...
        <div class="grid grid-cols-4 gap-6 mb-8">
            <div class="flex items-center justify-center flex-col bg-white rounded-xl shadow-sm border border-gray-200 p-6 cursor-pointer">
                ...
                <h1 class="text-2xl font-bold text-[#0F1729] mb-1"><?= $nb_clients ?></h1>
            ...
            <div class="flex items-center justify-center flex-col bg-white rounded-xl shadow-sm border border-gray-200 p-6 cursor-pointer">
                ...
                <h1 class="text-2xl font-bold text-[#0F1729] mb-1"><?= $nb_comptes ?></h1>
            ...
            <div class="flex items-center justify-center flex-col bg-white rounded-xl shadow-sm border border-gray-200 p-6 cursor-pointer">
                ...
                <h1 class="text-2xl font-bold text-[#0F1729] mb-1"><?= number_format($solde_total, 2, ',', ' ') ?></h1>
            ...
            <div class="flex items-center justify-center flex-col bg-white rounded-xl shadow-sm border border-gray-200 p-6 cursor-pointer">
                ...
                <h1 class="text-2xl font-bold text-[#0F1729] mb-1"><?= $nb_transactions ?></h1>
                <div class="flex items-center justify-center flex-col text-sm">
                    <span class="text-green-500 "><?= number_format($total_depot, 2, ',', ' ') ?> DH dépôts</span>
                    <span class="text-red-500"><?= number_format($total_retrait, 2, ',', ' ') ?> DH retraits</span>
                </div>
            </div>
        </div>
